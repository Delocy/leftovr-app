...
# Using gpt-3.5-turbo as fallback (change to gpt-4 if you have access)
llm = ChatOpenAI(
...
class LeftovrWorkflow:
    ...
    def _orchestrator_node(self, state: RecipeWorkflowState) -> Dict[str, Any]:
        ...
        # Classify query type using existing agent method
        query_classification = self.exec_chef.classify_query(llm, messages)
        ...
        if query_type in ["recipe", "general"]:
            ...
            preferences = self.exec_chef.extract_preferences(llm, messages)
    ...
    def _pantry_node(self, state: RecipeWorkflowState) -> Dict[str, Any]:
        ...
        # Use Executive Chef to extract ingredients from user message
        extracted = self.exec_chef.extract_ingredients(llm, user_msg)
    ...
    def _recommendation_node(self, state: RecipeWorkflowState) -> Dict[str, Any]:
        ...
        # Use Sous Chef's existing generate_recommendations method
        pantry_summary = {
            "inventory": inventory,
    ...
    def _customization_node(self, state: RecipeWorkflowState) -> Dict[str, Any]:
        ...
        # Use Sous Chef's existing adapt_recipe method
